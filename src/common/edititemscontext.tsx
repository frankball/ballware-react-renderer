...
>(({ children }: PropsWithChildren<EditItemsProviderProps>, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    validate: () => {
      return Object.keys(editors)
        .map(member => editors[member].validate())
        .every(Boolean);
    },
  }));
    ...
    if (editors) {
      return {
        getEditorOption: (dataMember, option) =>
          editors[dataMember]
            ? editors[dataMember].getOption(option)
            : undefined,
        setEditorOption: (dataMember, option, value) =>
          editors[dataMember]
            ? editors[dataMember].setOption(option, value)
            : undefined,
      } as EditUtil;
    }
  ...
  const editUtilForDetail = useCallback(
    (detailMember: string) => {
      if (editors) {
        return {
          getEditorOption: (dataMember, option) =>
            editors[`${detailMember}.${dataMember}`]
              ? editors[`${detailMember}.${dataMember}`].getOption(option)
              : undefined,
          setEditorOption: (dataMember, option, value) =>
            editors[`${detailMember}.${dataMember}`]
              ? editors[`${detailMember}.${dataMember}`].setOption(
                  option,
                  value
                )
              : undefined,
    ...
    if (
      ...
      setValue(previousValue => {
        return {
          ...
          getValue: dataMember =>
            dataMember
              ? getByPath(item as Record<string, unknown>, dataMember)
              : item,
          readOnly: () => mode === EditModes.VIEW,
          editorPreparing: (dataMember, layoutItem) =>
            editorPreparing(
              mode,
          ...
          editorValidating: (dataMember, identifier, value) =>
            editorValidating(
              mode,
              item as Record<string, unknown>,
              editUtil,
          ...
          detailGridCellPreparing: (dataMember, detailItem, options) =>
            detailGridCellPreparing(
              mode,
              item as Record<string, unknown>,
              detailItem,
          ...
          detailEditorEntered: (dataMember, detailItem, detailMember) =>
            editorEntered(
              mode,
              detailItem,
              editUtilForDetail(dataMember),
          ...
          detailEditorEvent: (dataMember, detailItem, detailMember, event) =>
            editorEvent(
              mode,
              detailItem,
              editUtilForDetail(dataMember),
  ...
  return (
    <EditItemsContext.Provider value={value}>
      {children}
